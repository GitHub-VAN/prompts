...
2. **测试验证**：修改后运行相关测试确保功能正常
3. **浏览器测试**：UI相关修改需要在浏览器中验证
4. **避免破坏性修改**：遵循 "Never break userspace" 原则
5. **简洁优先**：消除不必要的if/else分支，优化数据结构
...
5. **【验证修复】** 运行测试
   ```bash
   python manage.py test inventory.tests.test_battery
   ```
6. **【浏览器验证】** 测试UI功能
   ```python
